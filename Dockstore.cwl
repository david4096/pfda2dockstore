...
outputs:
  archived_results:
    ...
    outputBinding: {glob: archived_results/*}
    type: File
  extended_csv:
    doc: A CSV file with extended statistics as output by the comparator
    outputBinding: {glob: extended_csv/*}
    type: File
requirements:
- {class: DockerRequirement, dockerOutputDirectory: /data/out, dockerImageId: vcfeval-happy-comparison-vcfcheck}
